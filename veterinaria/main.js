...
function guardar() {
    ...
    if (bd == "agregar") {
        ...
        data.forEach((e, i) => {
            if (e.id === id2) {
                let estadoOriginal = e.estadoCita; // Guarda el estado original
                ...
                e.estadoCita = estadoOriginal;
            }
            let cardDiv = document.getElementById(`tarjeta_${e.id}`);
                cardDiv.innerHTML = ""; // Limpia el contenido
                pintarCita(cardDiv, e); // Vuelve a pintar la tarjeta
        });
    }
}

function pintarCita(cardDiv, cita) {
    cardDiv.classList.add('card');
    cardDiv.style.width = '18rem';

    if (cita.estadoCita === 'sita terminada') {
        cardDiv.classList.add('card-sita-terminada');
    } else if (cita.estadoCita === 'sita Cancelada') {
        cardDiv.classList.add('card-sita-cancelada');
    }

    let cardImg = document.createElement('img');
    cardImg.classList.add('card-img-top');
    cardImg.src = cita.imagen;
    cardImg.alt = 'Imagen de ' + cita.tipo;
    ...
    cardTitle.textContent = cita.mascota;

    let cardText = document.createElement('p');
    cardText.classList.add('card-text');
    let horafor = convertirhorafor(cita.hora);
    cardText.innerHTML = 'Propietario: <strong>' + cita.propietario + '</strong><br>Teléfono: <strong>' + cita.telefono + '</strong><br>Tipo: <strong>' + cita.tipo + '</strong><br>Fechas: <strong>' + cita.fecha + '</strong><br>Hora: <strong>' + horafor + '</strong><br>Síntomas: <strong>' + cita.sintomas + '</strong>';
...
function editar(cita) {
    ...
    data.forEach((e) => {
        document.getElementById(`tarjeta_${e.id}`).style.display = "none";
    });

    // Mostrar la tarjeta específica que se está editando
    document.getElementById(`tarjeta_${cita.id}`).style.display = "block";
    ...
    modal.addEventListener('hidden.bs.modal', function () {
        // Restaurar la visibilidad de todas las tarjetas al cerrar la edición
        data.forEach((e) => {
            document.getElementById(`tarjeta_${e.id}`).style.display = "block";
        });

        limpiar();
        
        // Restaurar el filtro original y re-pintar las tarjetas según el filtro
        if (filtroActual === "abiertas") {
            mostrarTarjetasAbiertas();
        } else if (filtroActual === "terminadas") {
            mostrarTarjetasTerminadas();
        } else if (filtroActual === "canceladas") {
            mostrarTarjetasCanceladas();
        }
    });
}
...
function pintar() {
    data.forEach((e, i) => {
        ...
        estadoSelect.classList.add('form-select', 'mb-3');
        estadoSelect.innerHTML = `
        <option value="sita por defecto">Estado de Cita</option>
        <option value="sita terminada">Cita Terminada</option>
        <option value="sita Cancelada">Cita Cancelada</option>
        `;
